...
class docker_helper:
    def __init__(self, firmweld_root, remove_image=False):
        self.firmweld_root = firmweld_root
        self.count = 0
        self.last_core = None
        self.__sync_status()
    ...
    def run_core(self, idx, mode, brand, firmware_path):
        ...
        cmd = """docker run -dit --rm \\
                ...
                --privileged=true \\
                --name {2} \\
                firmweld:latest""".format(self.firmweld_root,firmware_root,docker_name)
        ...
        sp.check_output(
            f'docker exec -it "{docker_name}" bash -lc "chmod -R 777 /work/FirmWELD"',
            shell=True
        )

        docker_mode = "-it" if mode == "-d" else "-id"
        cmd = "docker exec {0} \"{1}\" ".format(docker_mode, docker_name)
        cmd += "bash -c \"cd /work/FirmWELD &&"
        cmd += "./run.sh {0} {1} /work/firmwares/{2} ".format(mode,brand,firmware)
        if mode == "-d":
            cmd += "\""
        else:
            cmd += "2>&1 > /work/FirmWELD/scratch/{0}.log\" &".format(firmware)
        ...
        if mode == "-d":
            ...
            sp.check_output(cmd, shell=True)
        firmware_log = os.path.join(self.firmweld_root, "scratch", firmware + ".log")
        ...
        tgz_path = os.path.join(self.firmweld_root, "images", str(iid) + ".tar.gz")
        for i in range(300):
            time.sleep(1)
        ...
        work_dir = os.path.join(self.firmweld_root, "scratch", str(iid))
        if os.path.exists(work_dir):
            with open(os.path.join(work_dir, 'time_elapsed'), 'w') as f:
                f.write("%0.4f\n" % (time_elapsed))

            os.rename(os.path.join(self.firmweld_root, 'scratch',
    ...
    def check_result(self, firmware, docker_name, brand, iid, is_emulation=True):
        ...
        if is_emulation:
            result_path = "{}/scratch/{}/result".format(self.firmweld_root, iid)
            timeout = 2400
        else:
            result_path = "{}/analyses/analyses_log/{}/{}/result".format(self.firmweld_root, brand, iid)
            timeout = 3600
...
def main():
    LIST_SUFFIX = ('.list', '.txt')
    if len(sys.argv) < 2:
        print_usage(sys.argv[0])
        exit(1)

    firmweld_root=os.path.abspath('.')
    dh = docker_helper(firmweld_root, remove_image=False)
    ...
    if sys.argv[1] in ['-ec', '-ea']:
        ...
        if not os.path.exists(os.path.join(firmweld_root, "scratch")):
            os.mkdir(os.path.join(firmweld_root, "scratch"))

        brand = sys.argv[2]
        mode = '-' + sys.argv[1][-1]
        firmware_path = os.path.abspath(sys.argv[3])
        if os.path.isfile(firmware_path) and not firmware_path.endswith(LIST_SUFFIX):
            argv = (0, dh, mode, brand, firmware_path)
            runner(argv)

        elif (os.path.isfile(firmware_path) and firmware_path.endswith(LIST_SUFFIX)) \
            ...
            num_cores = min(30, mp.cpu_count(), len(firmwares))
