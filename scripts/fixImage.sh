...
# 
if $BUSYBOX grep -sq "IPAddress" /bin/goahead; then
    echo "Creating default IPAddress!"
    echo -n "2.65.87.200" > /firmadyne/libnvram.override/IPAddress
fi

if $BUSYBOX grep -sq "IPAddress" /bin/alphapd; then
    echo "Creating default IPAddress!"
    echo -n "2.65.87.200" > /firmadyne/libnvram.override/IPAddress
fi

if $BUSYBOX grep -sq "lan_ipaddr" /bin/alphapd; then
    echo "Creating default lan_ipaddr!"
    echo -n "2.65.87.200" > /firmadyne/libnvram.override/lan_ipaddr
fi

if [ -e /etc/init.d/uhttpd ]; then
    echo "[*] Patching /etc/init.d/uhttpd ..."
    $BUSYBOX chmod u+rw /etc/init.d/uhttpd

    if ! $BUSYBOX grep -q "set +e" /etc/init.d/uhttpd; then
        $BUSYBOX sed -i '1a set +e' /etc/init.d/uhttpd
    fi

    $BUSYBOX sed -i -r \
        's#(local[[:space:]]+[A-Za-z0-9_]+="\$\(uci [^)]*)(\)")#\1 2>/dev/null || echo no\2#' \
        /etc/init.d/uhttpd

    echo "[*] /etc/init.d/uhttpd patched."
fi

if [ -f /etc/init.d/lighttpd ] && \
   ${BUSYBOX} grep -q "web_feature=" /etc/init.d/lighttpd; then
    echo "[FIRMWELD] Patching /etc/init.d/lighttpd: force web_feature=1"
    ${BUSYBOX} chmod u+rw /etc/init.d/lighttpd 2>/dev/null
    ${BUSYBOX} sed -i 's|web_feature=$(wifisys_tool -g 1 -f 1)|web_feature=1|' /etc/init.d/lighttpd
fi
