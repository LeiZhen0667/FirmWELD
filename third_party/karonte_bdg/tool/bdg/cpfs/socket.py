...
from bdg.utils import are_parameters_in_registers, find_elf_files_containing, get_dyn_sym_addrs, get_string
        ...
        for role, data_key in zip(self._roles, self._data_keys):
            if role == Role.SETTER and data_key:
                for binding in self._bindings:
                    ...
                    packed_binding = bytes.fromhex(val)
                    candidate_bins = find_elf_files_containing(
                        self._fw_path, [binding[1], packed_binding]
                    )
